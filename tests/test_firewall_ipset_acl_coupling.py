...
can reach:
  ...
  * the packet-layer **app egress ipset** (`allowed-domains`), populated with the
    ...
    `monitor-ntfy` ipset, reachable only from the monitor sidecar's uid — never the
    app set. So the app ipset's domain set is exactly the ro+rw tiers.
...
def _render_acls(tmp_path, ro_domains: list, rw_domains: list) -> tuple:
...
def _squid_admits(host: str, ro_lines: list, rw_lines: list) -> bool:
...
def _partition(access: dict) -> tuple:
...
_ACCESS = {
    "example.com": "ro",
    "alignment.example.com": "ro",  # ro child of a ro parent -> suppressed, covered
    "a.b.example.com": "ro",  # deep ro descendant -> suppressed, covered
    "api.example.com": "rw",  # rw child of a ro parent -> exact rw escape
    "github.com": "ro",
...
def _random_access_map(rng: random.Random) -> dict:
    ...
    ro/rw/monitor tiers."""
    apexes = ["example.com", "github.com", "example.org", "co.uk", "hf.co"]
