...
widening the firewall — blocking is the point, and loosening access is a
deliberate human decision left out of the session.

The squid config lives in `squid-config.bash` (sourced via firewall-lib.bash),
so the exact text can be rendered and `squid -k parse`-validated by CI. These
tests assert the wiring and render the functions.
"""

import subprocess

from tests._helpers import dstdomain_covers, REPO_ROOT

FIREWALL_LIB = REPO_ROOT / "sandbox" / "firewall-lib.bash"

ERR_PAGE_NAME = "ERR_AGENT_SANDBOX_READONLY"
...
def test_rw_child_of_readonly_wildcard_is_spliced_not_bumped() -> None:
    """A rw domain (api.example.com) is often a subdomain of a read-only wildcard
...
def _write_ro_domains(tmp_path, domains: list) -> list:
...
def test_write_ro_domains_satisfies_squids_dstdomain_constraints(tmp_path) -> None:
    ...
    (b) NO emitted line is a subdomain of another emitted line — squid 6.x treats a
        dstdomain ACL holding both `.example.com` and `.alignment.example.com`
    ...
    inputs = [
        "example.com",
        "alignment.example.com",  # child of a listed parent -> must be suppressed
        "a.b.example.com",  # deep descendant of a listed parent -> suppressed
        "other.org",
