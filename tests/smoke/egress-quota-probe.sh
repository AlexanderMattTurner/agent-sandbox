...
# .github/scripts/run-firewall-probe.sh). Proves that EGRESS_QUOTA_MB is a real,
# byte-exact hard cap on outbound traffic to allowed domains: traffic flows until
# ~1 MB has crossed the quota rule, after which ALL further allowed-domain egress
# is REJECTed for the rest of the session. This is the e2e probe egress-rules.bash
# promises in its header — the only place the kernel `-m quota` counter is observed
# actually flipping allowed-domain egress from ACCEPT to REJECT once spent.
#
# Why a dummy interface and not loopback: install_egress_output_rules ACCEPTs
# OUTPUT to 127.0.0.0/8 and the sandbox subnet BEFORE the quota rule, so loopback
# traffic never reaches `-m quota` (it short-circuits the budget). We instead stand
# up a dummy interface bearing a genuinely PUBLIC ip (93.184.216.34 — outside every
# BOGON_CIDRS range), add that ip to the `allowed-domains` ipset, and serve a large
# body from an origin bound to it. Traffic to it is local (no external network) yet
# still traverses the OUTPUT chain and the quota rule, exactly as real
# allowed-domain egress would.
#
# Determinism: we instrument the kernel's `-m quota` byte counter, never timing.
# `iptables -Z OUTPUT` zeroes the rule counters; we then drive a deterministic byte
# volume and ASSERT, via `iptables -L OUTPUT -v -n -x`, that the over-quota REJECT
# rule's packet counter went 0 -> >0 (the budget was exhausted). The flip is the
# teeth: the SAME endpoint is reachable before the cap and REJECTed after; the only
# thing that changed is the byte budget. REJECT (icmp-admin-prohibited) fails the
# client instantly, so the post-quota outcome needs no polling.
#
...
# the exact OUTPUT chain production installs — there is no second copy to drift. We
# can't run the full init (a cold boot resolves live domains and fails closed on
# zero essentials, impossible with no external network), but the rule installation
# is one function, and that is what we drive here against a test ipset and
# EGRESS_QUOTA_MB=1.
#
...
# The function's own final `-j REJECT` is the catch-all, and only packets to
# PUBLIC_IP reach the ipset rules since it is the set's only member.
...
# The `-m set` match opens a netlink socket that needs CAP_NET_RAW; without it the
# install prints "Can't open socket to ipset" and silently drops the rule — without
# this guard the only symptom is a baffling downstream "origin did not come up".
...
# order reversed, the bulk packets would hit ESTABLISHED first, the quota would see
# only the SYN, and the REJECT counter would stay zero forever.
