...
Fails if any enrolled script is below 100% or is missing from the report (which would
mean its tests never invoked it under kcov — a silent hole). Also prints every active
`kcov-ignore` exclusion marker so none can hide unreviewed.

    python -m tests.kcov_gate <merged-cobertura.xml>
"""
...
def _uncovered_by_file(cobertura: Path) -> dict[str, list[int]]:
    """Map each enrolled repo-relative path -> sorted uncovered line numbers. A path
    absent from the report maps to [-1] as a sentinel for 'never traced'."""
    tree = ET.parse(cobertura)
    # kcov writes each class `filename` relative to a <source> dir (a bare basename for
    # our bin/ wrappers), so the enrolled relpath is recovered by joining the filename
    # onto each source, not from the filename alone.
...
def _enrolled_rel(filename: str, sources: list[str]) -> str | None:
    """The KCOV_GATED relpath a class maps to, or None if it isn't enrolled. kcov names
    the class by its basename and the directory in <source>, so try the filename both
    alone (already repo-relative or absolute) and joined onto each source dir, keeping
    the candidate that resolves to an enrolled path."""
