...
`cross_platform` marker from it). These tests keep it honest: every host file with
a GNU/BSD-divergent construct must be declared, and every declared file must exist
and have a non-opted-out covering test. See _xplat.py's docstring for why the set
is explicit rather than regex-derived.
"""
...
def test_guard_detects_an_undeclared_portable_host_file(
    ...
    fallback and no declaration is flagged by portable_host_files() and absent from
    XPLAT_HOST_FILES — proving the subset assertion isn't passing on air."""
    (tmp_path / "bin" / "lib").mkdir(parents=True)
    (tmp_path / "setup.sh").write_text("#!/usr/bin/env bash\n:\n")
    (tmp_path / "bin" / "agent-sandbox-fabricated").write_text(
        "#!/usr/bin/env bash\nstat -c '%a' x 2>/dev/null || stat -f '%Lp' x\n"
    )
    monkeypatch.setattr(_xplat, "REPO_ROOT", tmp_path)
    found = portable_host_files()
    assert "bin/agent-sandbox-fabricated" in found
    assert "bin/agent-sandbox-fabricated" not in XPLAT_HOST_FILES
