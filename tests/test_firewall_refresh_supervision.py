...
# covers: sandbox/dns-resolver.bash, sandbox/firewall-lib.bash
"""
...
def test_boot_gate_fails_closed_and_withholds_event_when_refresher_never_comes_up(
    ...
    as_trace line, leaving no firewall_refresh_supervised in the trace log."""
    heartbeat = tmp_path / "hb"
    pidfile = tmp_path / "pid"
    stderr = tmp_path / "err"
    ...
    # must time out and the gate must `exit` before reaching the as_trace announcement.
    gate = (
        "set -Eeuo pipefail\n"
        f"exec 2>'{stderr}'\n"
        f"source '{FIREWALL_LIB}'\n"
        ...
        "  exit 7\n"
        "fi\n"
        'as_trace "${TRACE_FIREWALL_REFRESH_SUPERVISED:-}" interval=5\n'
    )
