...
# Route enrolled bash wrappers through kcov when AGENT_SANDBOX_KCOV_OUT is set (the
...
def pytest_collection_modifyitems(
    ...
    *Derivation (always runs):* a test whose module's `# covers:` directive names a
    file in `XPLAT_HOST_FILES` (and is not opted out as Linux-only-behavior) is
    marked `cross_platform`, so the cross-platform matrix runs it on the macOS/BSD
    leg. This is the SSOT for "host file X must get BSD coverage" — see
    tests/_xplat.py — replacing per-file `pytestmark` lines that drift from the host
    source they guard.

    *Sharding (only when KCOV_SHARD_COUNT is set):* the kcov pass fans out across
    parallel CI jobs (kcov's DEBUG tracing is too slow to run serially, see
    ...
    script is still traced somewhere. A no-op unless the env vars are set. The hash
    is stable (md5, not Python's salted hash()) so all xdist workers agree."""
