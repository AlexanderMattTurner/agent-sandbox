...
- **Don’t call something an SSOT when it’s really duplication guarded by a drift test.** Two hand-edited files kept in step by a round-trip/“they must match” test are duplication _with a guard_, not a single source of truth — name it a drift guard and say so. A real SSOT has exactly **one** authored copy; any second representation a different runtime needs (e.g. a bash map an in-container script reads because it can’t parse the JSON, a JSON a JSON-only tool needs) must be **generated** from the source: a committed, `DO NOT EDIT` artifact regenerated by a script and enforced by a `--check` in CI **plus** a pre-commit hook, never hand-maintained. Reach for a drift guard only when a generator is genuinely infeasible, and call it what it is. (Worked example: `scripts/gen-trace-events.py` regenerating `sandbox/trace-events.bash` from `config/trace-events.json`.)
