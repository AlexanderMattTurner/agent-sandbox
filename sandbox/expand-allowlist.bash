...
# domains go live.
#
# Runs INSIDE the firewall container as root. The workload container is on the
...
# `agent-sandbox expand`, or `docker exec <firewall>` directly).
#
...
# a domain permanent, add it to the Workload record's egress_allowlist.
#
...
# Opt-in structured trace channel: announce the expansion ENGAGED on success, so a
# widened egress boundary is observable, never silent. No-op unless
# AGENT_SANDBOX_TRACE is set; tolerate a stripped image with a no-op fallback.
...
for arg in "$@"; do
  ...
  # path's workload allowlist reject the same malformed shapes. An operator typo
  # here is loud (exit), unlike a workload entry the build path merely skips.
...
# Announce the boundary WIDENED — metadata only (counts, never the domain names).
# `:-` keeps the no-trace fallback from tripping set -u; the no-op as_trace
# ignores the empty arg.
as_trace "${TRACE_FIREWALL_ALLOWLIST_EXPANDED:-}" domains="${#REQUESTED[@]}" resolved="${#resolved[@]}"
