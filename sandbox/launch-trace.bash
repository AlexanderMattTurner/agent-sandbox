...
# AGENT_SANDBOX_LAUNCH_TRACE, so an analyzer can difference consecutive marks. A no-op
...
launch_trace_init() {
  [[ -n "${AGENT_SANDBOX_LAUNCH_TRACE:-}" ]] || return 0
  : >>"$AGENT_SANDBOX_LAUNCH_TRACE" 2>/dev/null || return 0
  chmod 0666 "$AGENT_SANDBOX_LAUNCH_TRACE" 2>/dev/null || true
}
...
launch_trace_mark() {
  ...
  printf '%s\t%s\n' "$1" "$_ms" >>"$AGENT_SANDBOX_LAUNCH_TRACE" 2>/dev/null || true
}
