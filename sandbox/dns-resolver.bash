...
batch_resolve_a() {
  ...
  local concurrency="${AGENT_SANDBOX_DNS_BATCH_CONCURRENCY:-1}"
...
#
# AGENT_SANDBOX_DNS_RETRY_BACKOFF overrides the first-retry delay (seconds; doubles
...
resolve_a_with_retries() {
  ...
  local delay="${AGENT_SANDBOX_DNS_RETRY_BACKOFF:-1}"
...
# Override with AGENT_SANDBOX_DNS_FALLBACK_RESOLVERS (space-separated IPv4) to point at an
...
fallback_resolvers() {
  ...
  local IFS=$' \t\n'
  read -ra list <<<"${AGENT_SANDBOX_DNS_FALLBACK_RESOLVERS-8.8.8.8 1.1.1.1}"
...
cold_boot_resolve() {
  local batch_size="$1"
  shift
  AGENT_SANDBOX_DNS_BATCH_CONCURRENCY="${AGENT_SANDBOX_DNS_BATCH_CONCURRENCY:-4}" \
    AGENT_SANDBOX_DNS_RETRY_BACKOFF="${AGENT_SANDBOX_DNS_RETRY_BACKOFF:-0}" \
...
# resolver momentarily sheds (a CDN-fronted host) used to
...
# Never returns. as_trace is guarded so the function is sourceable outside the container
...
supervise_refresher() {
  ...
  while true; do
    ...
    if command -v as_trace >/dev/null 2>&1; then
      as_trace "${TRACE_FIREWALL_REFRESH_DIED:-}" rc="$rc" restarts="$restarts"
    fi
