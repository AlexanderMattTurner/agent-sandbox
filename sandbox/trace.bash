...
# The generated event NAME constants plus the per-event layer/level maps as_trace
...
# mandatory — as_trace references their symbols under set -u — and they are always
...
# _cg_trace_threshold — numeric verbosity from AGENT_SANDBOX_TRACE: 0 off, 1 info,
# 2 debug. Unknown, empty, or "off" → 0. Mirrors trace_threshold in the JS/Python shims.
_cg_trace_threshold() {
  case "${AGENT_SANDBOX_TRACE:-}" in
  debug | 2) echo 2 ;;
...
# as_trace EVENT [k=v ...] — emit one JSON line
#   {"ts":<epoch_ms>,"layer":"<layer>","event":"<EVENT>","level":"<level>",...<kv>}
# for EVENT, carrying the metadata k=v fields, to AGENT_SANDBOX_TRACE_FILE when set else
...
as_trace() {
  [[ -n "${AGENT_SANDBOX_TRACE:-}" ]] || return 0
  local event="${1:-}"
  ...
  local need=1
  [[ "$level" == "debug" ]] && need=2
  (($(_cg_trace_threshold) >= need)) || return 0
  ...
  if [[ -n "${AGENT_SANDBOX_TRACE_FILE:-}" ]]; then
    printf '%s\n' "$line" >>"$AGENT_SANDBOX_TRACE_FILE" 2>/dev/null || true
  else
