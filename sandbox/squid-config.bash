...
# impossible here.
#
# Batches resolve SEQUENTIALLY by default (AGENT_SANDBOX_DNS_BATCH_CONCURRENCY=1). A larger
...
# AGENT_SANDBOX_DNS_BATCH_CONCURRENCY=N to run up to N batches at once — the initial build
...
write_ro_domains() {
  ...
  # a wildcard parent (`.example.com`) and a subdomain of it
  # (`.sub.example.com`) as a FATAL config error and refuses to start. The
...
# rw domain (e.g. api.example.com) is often a subdomain of a read-only wildcard
# (.example.com from a ro `example.com`); squid matches the wildcard against
...
# of a read-only wildcard (api.example.com under .example.com); listed exactly
...
# to the workspace repo (widen with AGENT_SANDBOX_GH_TOKEN_REPOS), so a push to any other
...
# ERR_AGENT_SANDBOX_READONLY into below, so it resolves regardless of Accept-Language.
...
deny_info ERR_AGENT_SANDBOX_READONLY readonly_domains
...
# Exclude rw_domains so a rw child of a read-only wildcard (api.example.com under
# .example.com) is not method-restricted; it is spliced below in any case.
...
ERRPAGE
  cat >"$SQUID_ERR_DIR/ERR_AGENT_SANDBOX_READONLY" <<'ERRPAGE'
<!DOCTYPE html>
