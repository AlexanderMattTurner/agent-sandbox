...
# rejects the VM's /tmp symlink, orbstack/orbstack#2362; interactive workloads hang
# in an epoll_pwait loop under runsc on ARM64), so it is
...
# surfaces only at the first `agent-sandbox` launch — minutes later, with none of
...
# (bin/agent-sandbox); on failure sandbox_ok stays false so the caller's FATAL gate
...
setup_linux_sandbox() {
  ...
  if [[ "${NO_SUDO:-false}" == true ]]; then
    if docker_provider_is_docker_desktop; then
      warn "Docker Desktop is the active Docker provider — it can't host the agent-sandbox sandbox."
  ...
  # nothing, and the Desktop VM hangs workloads under gVisor/Kata anyway. Refuse loud
  ...
  if docker_provider_is_docker_desktop; then
    ...
    warn "  Its daemon runs in a VM that ignores /etc/docker/daemon.json and hangs workloads under gVisor/Kata."
