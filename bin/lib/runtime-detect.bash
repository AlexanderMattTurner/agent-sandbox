...
# Single source of truth for the container runtime, sourced by the agent-sandbox
# launcher (exports CONTAINER_RUNTIME) and bin/check-sandbox-runtime.bash.
...
# where it is genuinely absent, run unbounded. Tune AGENT_SANDBOX_DOCKER_PROBE_TIMEOUT.
# Single source of the bound (and its default) shared by both probes below.
_docker_bounded() {
  if command -v timeout >/dev/null 2>&1; then
    timeout "${AGENT_SANDBOX_DOCKER_PROBE_TIMEOUT:-10}" "$@"
  else
    "$@"
  fi
}
...
# several times (agent-sandbox, check-sandbox-runtime, setup's engine checks). Cache
...
docker_daemon_reachable() {
  [[ "${_AGENT_SANDBOX_DOCKER_REACHABLE:-}" == "1" ]] && return 0
  docker info >/dev/null 2>&1 || return 1
  _AGENT_SANDBOX_DOCKER_REACHABLE=1
  return 0
}
...
# $XDG_CONFIG_HOME/agent-sandbox/, the established convention for persisted prefs.
container_runtime_pref_file() {
  printf '%s/agent-sandbox/container-runtime\n' "${XDG_CONFIG_HOME:-$HOME/.config}"
}
...
# (which owns as_error) turns that into a loud fail-closed abort. runc promises no
...
# work, so setup.bash's WSL2 note and agent-sandbox-doctor agree instead of one
...
# Returns 0 if Docker can RELIABLY launch a workload under $runtime, 1 if not.
...
# Desktop's LinuxKit VM and `docker info` will list it, but interactive
# workloads are known to hang indefinitely in an epoll_pwait loop under runsc on
# Docker Desktop (reproducible on macOS ARM64; Linux Docker Desktop has the same
# VM architecture). Compose then blocks forever on healthcheck deps. Detect Docker
...
# read-only (lima-vm/lima#4053); Docker Desktop hangs workloads under runsc.
...
# Desktop VM hangs workloads under gVisor/Kata regardless. setup_linux_sandbox gates
...
detect_container_runtime() {
  ...
  then
    ...
    # (orbstack/orbstack#2362), and interactive workloads hang in an epoll_pwait
    # loop under runsc on ARM64. Default to runc
...
runtime_isolation_summary() {
  ...
  *)
    ...
    ISOLATION_DOWNGRADE="unrecognized container runtime '$rt' — run agent-sandbox-doctor"
    ;;
  esac
