...
#      capturing the seeded tree, on branch sandbox/<name>. The workload commits on top.
...
# directory AND really is owner-only. The seed-branches store holds the
...
# command exit status (success means the post-condition holds, not exit 0): `mkdir -p`
...
# directory would otherwise pass the mode check while writing plaintext where the attacker
# can read it — reject a symlinked `$dir` outright (checked before mkdir and re-checked
# between mkdir and chmod, so a symlink planted mid-call is refused before chmod follows it)
# and verify the resulting directory's owner uid matches ours, not just its mode. Bash has no
...
# check-to-use race, they cannot eliminate it. The files themselves
...
worktree_secure_mkdir() {
  ...
  if [[ -L "$dir" ]]; then
    as_error "worktree seed: refusing to use $dir — it is a symlink, not a real directory (a symlinked store could point at a location outside your control)"
    return 1
  fi
  ...
  if [[ ! -d "$dir" ]]; then
    as_error "worktree seed: could not create the owner-only store directory $dir"
    return 1
  fi
  mode="$(stat -c '%a' "$dir" 2>/dev/null || stat -f '%Lp' "$dir" 2>/dev/null)"
  if [[ -z "$mode" || $((8#$mode & 8#077)) -ne 0 ]]; then
    as_error "worktree seed: could not lock the plaintext store directory $dir to owner-only (mode ${mode:-unknown}); another local user may be able to read your uncommitted source"
    return 1
  fi
  owner="$(stat -c '%u' "$dir" 2>/dev/null || stat -f '%u' "$dir" 2>/dev/null)"
  if [[ -z "$owner" || "$owner" != "$(id -u)" ]]; then
    as_error "worktree seed: refusing $dir — owned by uid ${owner:-unknown}, not you (uid $(id -u)); another local user may control this directory"
    return 1
  fi
}
...
# ephemeral teardown sets WORKTREE_EXTRACT_RUNNER=as_run_detached (the same shield the
...
worktree_relocate_wip_outside_scratch() {
  ...
  if ! worktree_secure_mkdir "$persist_dir" || ! mv "$scratch_wip" "$dest" ||
    ...
    as_error "worktree seed: could not relocate the launch-time WIP patch out of the scratch dir into $persist_dir"
    return 1
  fi
  printf '%s\n' "$dest"
}
...
worktree_reseed_container() {
  ...
  '; then
    as_error "worktree seed: could not re-seed /workspace with the current working tree in $container_id"
    return 1
  fi
}
...
worktree_stamp_seed_fingerprint() {
  ...
  if ! head="$(git -C "$repo_root" rev-parse HEAD 2>/dev/null)"; then
    as_error "worktree seed: could not read HEAD of $repo_root to stamp the seed fingerprint"
    return 1
  fi
  ...
  ' sh "$head"; then
    as_error "worktree seed: could not stamp the seed fingerprint in $container_id"
    return 1
  fi
}
...
worktree_host_apply() {
  ...
  if [[ -s "$wip_patch" ]]; then
    ...
    if ! _wt_run git -C "$wt_dir" commit -q --no-verify -m "chore: uncommitted changes at session start"; then
      as_error "worktree extract: could not commit your uncommitted changes onto $branch"
      return 1
    fi
  fi
  if [[ -s "$agent_mbox" ]]; then
    if ! _wt_run git -C "$wt_dir" am "$agent_mbox"; then
      ...
      as_error "worktree extract: could not apply the agent's commits onto $branch."
      as_error "the agent's work is preserved — resolve and re-run: git -C $wt_dir am $agent_mbox"
      return 1
    fi
  fi
}
...
worktree_extract_to_host() {
  ...
  if ! (umask 077 && worktree_container_extract "$container_id" "$base_ref" >"$agent_mbox"); then
    as_error "worktree extract: could not read the agent's commits out of $container_id"
    return 1
  fi
  worktree_host_apply "$repo_root" "$base_commit" "$branch" "$wt_dir" "$wip_patch" "$agent_mbox"
}
...
# AGENT_SANDBOX_WORKTREE=1 branches (which are sandbox/<adj>-<noun> with no hex suffix).
seed_branch_name() {
  printf 'sandbox/%s\n' "${1#ephemeral-}"
}
