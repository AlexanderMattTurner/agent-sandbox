...
# Per-session sandbox /24 so multiple agent-sandbox sessions run concurrently.
#
...
# its /tmp fallback is world-writable and shared, so a bare /tmp/agent-sandbox would
...
if [[ -z "${SANDBOX_NET_RESERVE_DIR:-}" ]]; then
  if [[ -n "${XDG_RUNTIME_DIR:-}" ]]; then
    SANDBOX_NET_RESERVE_DIR="$XDG_RUNTIME_DIR/agent-sandbox"
  else
    SANDBOX_NET_RESERVE_DIR="/tmp/agent-sandbox-$(id -u)"
  fi
fi
...
prune_stale_sandbox_networks() {
  ...
  # GC_DRY_RUN=1 (agent-sandbox gc --dry-run) previews instead of removing. The
...
# world-writable /tmp fallback a co-tenant who pre-created /tmp/agent-sandbox-$UID
...
_assert_reserve_dir_ours() {
  ...
  [[ "$owner" == "$EUID" ]] && return 0
  as_error "sandbox reservation dir $dir is owned by uid ${owner:-unknown}, not you ($EUID); refusing to use it. Remove it (a co-tenant's leftover or a hijack attempt) and relaunch."
  exit 1
}
...
_assert_reserve_dir_mode() {
  ...
  [[ "$mode" == "700" ]] && return 0
  as_error "sandbox reservation dir $dir is mode ${mode:-unknown}, not 700; refusing to use it (a co-tenant could read your session-subnet picks). Remove it and relaunch."
  exit 1
}
...
export_sandbox_subnet() {
  ...
  if [[ -z "$octet" ]]; then
    as_error "all $((SANDBOX_NET_MAX_THIRD_OCTET + 1)) sandbox subnets ($(_sandbox_subnet 0) .. $(_sandbox_subnet "$SANDBOX_NET_MAX_THIRD_OCTET")) are in use; close a session, or raise SANDBOX_NET_MAX_THIRD_OCTET in bin/lib/sandbox-net.bash."
    exit 1
  fi
  subnet="$(_sandbox_subnet "$octet")"
